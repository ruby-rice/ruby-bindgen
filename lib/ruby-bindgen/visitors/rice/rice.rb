  ...
  module Visitors
    class Rice
      ...
      def visit_conversion_function(cursor)
        ...
        return unless [:cursor_class_decl, :cursor_class_template, :cursor_struct].include?(cursor.lexical_parent.kind)
      ...
      def visit_enum_decl(cursor)
        unless cursor.anonymous? || cursor.private?
          under = cursor.ancestors_by_kind(:cursor_class_decl, :cursor_struct, :cursor_namespace).first
      ...
      def visit_macro_definition(cursor)
        ...
        self.render_cursor(cursor, "constant", :name => tokens.tokens[0].spelling)
      end
