...
module RubyBindgen
  module Visitors
    class Rice
      ...
      def visit
        self.visit_translation_unit(@translation_unit.cursor)
      end

      def render(cursor, kind = nil, separator = "\n")
        results = Array.new
        cursor.each(false) do |child_cursor, parent_cursor|
          ...
          if kind && child_cursor.kind != kind
            next :continue
          end
        ...
        # Remote consecutive blank lines
        result.gsub!(/^($\n^){2,}/, "\n")

        result
      end

      def render_children(cursor, kind = nil, separator = "\n")
        @indentation += 2
        result = render(cursor, kind, separator)
      ...
      def visit_function(cursor)
        ...
        result << self.render_cursor(cursor, :parameter_types => parameter_types)
        result.join("\n")
      end

      def visit_class_decl(cursor)
        under = cursor.ancestors_by_kind(:cursor_class_decl, :cursor_struct, :cursor_namespace).first
        base = cursor.find_by_kind(false, :cursor_cxx_base_specifier).first
        self.render_cursor(cursor, :under => under, :base => base)
      end

      def visit_cxx_access_specifier(cursor)
        # Do nothing
      end

      def visit_cxx_base_specifier(cursor)
        # Do nothing
      end
      ...
      def visit_struct(cursor)
        ...
        result << self.render_cursor(cursor)
        result.join("\n")
      end
      ...
      def visit_field_decl(cursor)
        ...
        self.render_cursor(cursor, ffi_type: ffi_type)
      end

      def visit_translation_unit(cursor)
        @stream << render_cursor(cursor)
      end
      ...
      def render_cursor(cursor, local_variables = {})
        template_path = figure_template(cursor)
        render_template(template_path, local_variables.merge(:cursor => cursor))
      end

      def render_template(template_path, local_variables = {})
        ...
        template.result(b).rstrip
      end

      def render_callback(name, parameter_types, result_type)
        template_path = File.join(__dir__, "callback.erb")
        render_template(template_path, :ruby => name, :parameter_types => parameter_types,
